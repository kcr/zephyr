...
#ifdef DEBUG
const char version[] = "Zephyr server (DEBUG) $Revision: 3.25 $";
#else
const char version[] = "Zephyr server $Revision: 3.25 $";
#endif

#if !defined (lint) && !defined (SABER)
static const char rcsid_version_c[] =
    "$Id: version.c,v 3.25 1995-07-18 20:29:16 ghudson Exp $";
...
get_version()
{
  ...
  if (vers_buf[0] == '\0') {
#ifdef DEBUG
    sprintf(vers_buf,"Zephyr Server (DEBUG) $Revision: 3.25 $: %s",
	    ZSERVER_VERSION_STRING);
#else
    sprintf(vers_buf,"Zephyr Server $Revision: 3.25 $: %s",
	    ZSERVER_VERSION_STRING);
#endif /* DEBUG */
